...
class ParticleFilter:
    ...
    def update_particles_move_continuous(self, robot_ground_truth, amount):
        ...
        mu = robot_ground_truth.move_probabilities["move_continuous"]["mean"]
        sigma = robot_ground_truth.move_probabilities["move_continuous"]["sigma"]
        jitter = 0.01

        for i in range(0, np.size(self.particles)):
            self.particles[i] += (amount + np.random.normal(mu, sigma))
            if self.particles[i] > 1:
                self.particles[i] = 1 - abs(np.random.normal(0.0, jitter))
            elif self.particles[i] < 0:
                self.particles[i] = 0 + abs(np.random.normal(0.0, jitter))
    ...
    def calculate_weights_door_sensor_reading(self, world_ground_truth, robot_sensor, sensor_reading):
        ...
        no_door_if_door = 1 - robot_sensor.sensor_probabilites["door"]['door if door']
        door_if_no_door = robot_sensor.sensor_probabilites["door"]['door if not door']
        no_door_if_no_door = 1 - robot_sensor.sensor_probabilites["door"]['door if not door']

        for i, p_x in enumerate(self.particles):
            is_in_front_of_door = world_ground_truth.is_location_in_front_of_door(p_x)
            if is_in_front_of_door:
                if sensor_reading:
                    self.weights[i] = door_if_door
                else:
                    self.weights[i] = no_door_if_door
            else:
                if sensor_reading:
                    self.weights[i] = door_if_no_door
                else:
                    self.weights[i] = no_door_if_no_door
    ...
    def calculate_weights_distance_wall(self, robot_sensors, dist_reading):
        ...
        # YOUR CODE HERE
        sigma = robot_sensors.sensor_probabilites["distance_wall"]['sigma']

        for i, x in enumerate(self.particles):
            self.weights[i] = gaussian(dist_reading, x, sigma)
    ...
    def resample_particles(self):
        ...
        #normalizing the weights, then creating a new array that is the cumulative sum
        self.weights /= np.sum(self.weights)
        cumulative_sum = np.cumsum(self.weights)
        rand_nums = np.random.rand(self.particles.size)

        #found this lovely npfunction: np.searchsorted
        indices = np.searchsorted(cumulative_sum, rand_nums)
        self.particles = self.particles[indices]

        #setting the particle weights back to uniform
        self.weights = np.full(self.weights.size, 1/self.weights.size)
