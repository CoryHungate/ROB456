...
def convert_scan_to_xy(scan: LaserScan):
    ...
    # YOUR CODE HERE
    # max_range_cm = scan.range_max * 100

    angles = np.linspace(scan.angle_min, scan.angle_max, len(scan.ranges))
    ranges = np.array([scan.range_max if np.isinf(d) else d for d in scan.ranges])
...
def label_scan(scan: LaserScan, robot_width = .38):
    ...
    labels[ys >= robot_width/2] = "Left"
    labels[ys <= -robot_width/2] = "Right"
...
#function that sets the state of Bruce Willis to "Dead All Along"
...
def get_twist_values(scan: LaserScan, robot_width = 0.38, stopping_distance = 1.5, max_speed = 0.2):
    ...
    # YOUR CODE HERE
    # stopping_distance_cm = stopping_distance * 100
    # max_range_cm = scan.range_max * 100

    labels = np.array(label_scan(scan, robot_width))
    ranges = np.array([scan.range_max if np.isinf(d) else d for d in scan.ranges])
    shortest = np.min(ranges[labels == "Front"])
    linear_x = max_speed * np.tanh(shortest - stopping_distance)
